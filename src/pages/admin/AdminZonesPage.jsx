...
export default function AdminZonesPage() {
  ...
  const [form, setForm] = useState({ name: '', code: '', description: '', state: 'Kwara' });
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', code: '', description: '', state: 'Kwara' }); setShowModal(true); };
  const openEdit = (z) => { setEditing(z); setForm({ name: z.name, code: z.code, description: z.description || '', state: z.state || '' }); setShowModal(true); };
        ...
        <div className="grid-3">
          {zones.map(z => (
            <div key={z.id} className="card" style={{ borderLeft: '4px solid var(--color-primary)' }}>
              ...
              {z.state && <p style={{ fontSize: 13, color: 'var(--color-text-muted)', margin: '0 0 6px' }}>{z.state}</p>}
